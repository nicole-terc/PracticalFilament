...
class AndroidFilamentEngine(
    ...
    override fun loadMesh(path: String, materialInstanceHandle: Int, scale: Float): Int {
        ...
        val instance = materialInstances[materialInstanceHandle] ?: return -1
        val buffer = loadAssetBuffer(path) ?: return -1
        val mesh = MeshLoader.loadFilamesh(buffer, eng, instance, scale) ?: return -1
        indexBuffers.add(mesh.indexBuffer)
        vertexBuffers.add(mesh.vertexBuffer)
        scene?.addEntity(mesh.entity)
        val handle = nextHandle++
        renderables[handle] = mesh.entity
        renderableBuffers[handle] = RenderableBuffers(mesh.vertexBuffer)
        return handle
    }
