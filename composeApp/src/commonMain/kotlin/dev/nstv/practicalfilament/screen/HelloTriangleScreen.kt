    ...
    SampleScreenLayout(
        ...
        view = {
            FilamentView(
                ...
                lights = listOf(
                    LightConfig(type = dev.nstv.practicalfilament.filament.LightType.DIRECTIONAL, intensity = 10_000f)
                ),
                backgroundColor = Color(0.01f, 0.02f, 0.03f, 1f),
                onEngineReady = { readyEngine ->
                    engine = readyEngine
                    if (!readyEngine.supportsMaterialBuilder) {
                        supportNotice = "Hello Triangle currently needs runtime material compilation on this platform."
                        return@FilamentView
                    }
                    val materialHandle = readyEngine.buildMaterial(TriangleMaterialSource, shadingModel = "unlit")
                    if (materialHandle <= 0) {
        ...
        controls = {
            ...
            androidx.compose.material3.Text("Rotation Speed")
            Slider(
                value = rotationSpeed,
                valueRange = 0f..120f,
