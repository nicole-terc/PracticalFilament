        ...
        view = {
            FilamentView(
                ...
                onEngineReady = { readyEngine ->
                    engine = readyEngine
                    val materialHandle = readyEngine.loadMaterial(Res.getUri(TexturedObjectMaterialPath))
                    ...
                    val textureHandle = readyEngine.loadTexture(Res.getUri(TexturedObjectTexturePath))
                    if (textureHandle <= 0) {
                        notice = "Texture loading failed on this platform."
                        return@FilamentView
                    }
                    readyEngine.setTextureParameter(instanceHandle, samplerName, textureHandle)
                    renderableHandle = readyEngine.createPlaneRenderable(instanceHandle, width = 2.4f, height = 2.4f)
                },
            )
        },
        controls = {
